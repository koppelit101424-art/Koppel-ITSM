<?php
include __DIR__ . '/../../../includes/auth.php';
  include __DIR__ . '/../../../includes/db.php';

if (!isset($_GET['id'])) {
    die("Invalid request.");
}

$transaction_id = intval($_GET['id']);

// IMPORTANT: include t.date_returned and i.date_received in the SELECT
$sql = "SELECT 
            t.transaction_id, t.action, t.quantity, t.action_date, t.date_returned, t.remarks,
            i.name AS item_name, i.item_code, i.brand, i.model, i.serial_number, i.description, i.date_received,
            u.fullname, u.emp_id, u.department, u.position, u.area, u.company
        FROM transaction_tb t
        JOIN item_tb i ON t.item_id = i.item_id
        JOIN user_tb u ON t.user_id = u.user_id
        WHERE t.transaction_id = ?";

$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $transaction_id);
$stmt->execute();
$result = $stmt->get_result();

if ($result->num_rows === 0) {
    die("Transaction not found.");
}
$row = $result->fetch_assoc();
$stmt->close();

// Safe date received formatting
$dateReceivedRaw = $row['date_received'] ?? null;
$dateReceivedDisplay = 'N/A';
if (!empty($dateReceivedRaw) && $dateReceivedRaw !== '0000-00-00' && $dateReceivedRaw !== '0000-00-00 00:00:00') {
    $ts3 = strtotime($dateReceivedRaw);
    if ($ts3 !== false) {
        $dateReceivedDisplay = date('m-d-Y', $ts3);
    }
}

// Safe returned date formatting
$dateReturnedRaw = $row['date_returned'] ?? null;
$dateReturnedDisplay = 'N/A';
if (!empty($dateReturnedRaw) && $dateReturnedRaw !== '0000-00-00' && $dateReturnedRaw !== '0000-00-00 00:00:00') {
    $ts = strtotime($dateReturnedRaw);
    if ($ts !== false) {
        $dateReturnedDisplay = date('m-d-Y', $ts);
    }
}

// Safe issued/transaction date formatting
$actionDateRaw = $row['action_date'] ?? null;
$actionDateDisplay = '';
if (!empty($actionDateRaw) && $actionDateRaw !== '0000-00-00' && $actionDateRaw !== '0000-00-00 00:00:00') {
    $ts2 = strtotime($actionDateRaw);
    $actionDateDisplay = $ts2 !== false ? date('m-d-Y', $ts2) : '';
}

// Normalize company for logo selection
$company = strtoupper(trim($row['company'] ?? ''));
switch ($company) {
    case 'KOPPEL INC.':
        $logo = '../asset/img/Koppel.jpg';
        break;
    case 'HEEC':
        $logo = '../asset/img/HEEC.jpg';
        break;
    case 'HIMC':
        $logo = '../asset/img/HIMC.png';
        break;
    case 'HI-AIRE':
        $logo = '../asset/img/HIAIRE.png';
        break;
    default:
        $logo = '../asset/img/default.jpg'; // fallback logo
        break;
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <title>ITSM - Print Return</title>
  <link rel="icon" href="../asset/img/Koppel_bip.ico">
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
  <style>
    body { font-size: 11px; font-family: Arial, Helvetica, sans-serif; margin: 0; padding-bottom: 70px; }
    .header { text-align: center; margin-top: 10px; }
    .header .title { font-weight: bold; font-size: 16px; }
    .logo { margin-bottom: 5px; }
    footer.sticky-footer {
      position: fixed; bottom: 0; left: 0; right: 0; width: 100%;
      background: #fff; z-index: 1000; padding: 7px 0;
    }
    footer.sticky-footer table { width: 100%; text-align: center; font-size: 10px; border-collapse: collapse; border: 2px solid #000; }
    footer.sticky-footer td { padding: 10px; }
    @media print { footer.sticky-footer { position: fixed; bottom: 0; } }
    pre { margin: 0; font-family: inherit; white-space: pre-wrap; }
  </style>
</head>
<body onload="window.print()">

<div class="header">
  <img src="<?= htmlspecialchars($logo) ?>" alt="<?= htmlspecialchars($company) ?>" height="80" class="logo">
  <p>Hyatt Centre Ortigas Ave., Brgy. Wack Wack, Mandaluyong City, Metro Manila</p>
  <hr style="border: none; border-top: 2px dashed black;">
  <h1><strong>RETURNED ASSET FORM</strong></h1>
  <hr style="border: none; border-top: 2px dashed black;">
</div>

<table border="1" cellpadding="10" cellspacing="0" style="width:100%; border-collapse:collapse; margin:20px auto; border:2px solid #000;">
  <tr>
    <td style="width:50%; border:2px solid #000; text-align:left; padding:6px; font-size:10px;">
      <strong>Name: <?= htmlspecialchars($row['fullname']) ?></strong>
    </td>
    <td style="width:50%; border:2px solid #000; text-align:left; padding:6px; font-size:10px;">
      <strong>Position: <?= htmlspecialchars($row['position']) ?></strong>
    </td>
  </tr>
  <tr>
    <td style="width:50%; border:2px solid #000; text-align:left; padding:6px; font-size:10px;">
      <strong>Employee No.: <?= htmlspecialchars($row['emp_id']) ?></strong>
    </td>
    <td style="width:50%; border:2px solid #000; text-align:left; padding:6px; font-size:10px;">
      <strong>Area: <?= htmlspecialchars($row['area']) ?></strong>
    </td>
  </tr>
</table>

<table border="1" cellpadding="6" cellspacing="0" style="width:100%; border-collapse:collapse; text-align:center; border:2px solid #000; font-size:10px;">
  <tr>
    <th style="border:none;">No</th>
    <th style="border:none;">Item Code</th>
    <th style="border:none;">Unit Type</th>
    <th style="border:none;">Brand</th>
    <th style="border:none;">Model</th>
    <th style="border:none;">Serial</th>
    <th style="border:none;">Qty</th>
    <th style="border:none;">Date Received</th>
    <th style="border:none;">Date Returned</th>
  </tr>
  <tr>
    <td style="border:none;">1</td>
    <td style="border:none;"><?= htmlspecialchars($row['item_code']) ?></td>
    <td style="border:none;"><?= htmlspecialchars($row['item_name']) ?></td>
    <td style="border:none;"><?= htmlspecialchars($row['brand']) ?></td>
    <td style="border:none;"><?= htmlspecialchars($row['model']) ?></td>
    <td style="border:none;"><?= htmlspecialchars($row['serial_number']) ?></td>
    <td style="border:none;"><?= htmlspecialchars($row['quantity']) ?></td>
    <td style="border:none;"><?= $actionDateDisplay ?></td>
    <td style="border:none;"><?= $dateReturnedDisplay ?></td>
  </tr>
  <tr>
    <td colspan="9" style="border:none; text-align:left; padding-top:12px; padding-bottom:20px;">
      <strong>Description:</strong><br><br>
      <pre><?= htmlspecialchars($row['description']) ?></pre>
    </td>
  </tr>
</table>

<p><strong>Reason / Remarks:</strong><br>
  <pre><?= htmlspecialchars($row['remarks']) ?></pre>
</p>

<footer class="sticky-footer">
  <table>
    <tr>
      <td>Prepared by:<br><br><br><strong>IT Staff/Assistant</strong></td>
      <td>Checked by:<br><br><br><strong>IT Supervisor</strong></td>
      <td>Noted by:<br><br><br><strong>HR Department</strong></td>
      <td>Noted by:<br><br><br><strong>IT Director</strong></td>
      <td>Returned by:<br><br><strong><?= htmlspecialchars($row['fullname']) ?><br><?= htmlspecialchars($row['position']) ?></strong></td>
    </tr>
  </table>
</footer>

</body>
</html>
<?php $conn->close(); ?>
