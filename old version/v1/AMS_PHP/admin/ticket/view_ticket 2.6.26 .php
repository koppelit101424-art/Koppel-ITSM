...
<style>
.chat-box { height:200px; overflow-y:auto; background:#f8f9fa; padding:10px; border-radius:6px; }
...
document.querySelectorAll('.live-update').forEach(el => {
    ...
    // lock all dropdowns if not assigned
    if (!canEditStatus) {
        el.disabled = true;
    }
    ...
    el.addEventListener('change', () => {
        ...
        // rebuild status workflow immediately
    ...
    assignBtn.addEventListener('click', () => {
        ...
        .then(data => {
            if (data.success) {
                ...
                // unlock all dropdowns instantly
                document.querySelectorAll('.live-update').forEach(el => {
                    el.disabled = false;
                });
            }
