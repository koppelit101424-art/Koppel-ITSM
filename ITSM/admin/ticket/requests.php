...
<td onclick="event.stopPropagation();" style="width:220px; padding:0;">
    <?php
        $statusColors = [
            'pending'         => 'warning',
            'proceed request' => 'success',
            'rejected'        => 'danger',
            'completed'       => 'primary',
            'canceled'        => 'secondary',
        ];

        $color = $statusColors[strtolower($currentStatus)] ?? 'secondary';
    ?>

    <span
        class="badge bg-<?= $color ?> request-status w-100 h-100 d-flex align-items-center justify-content-center"
