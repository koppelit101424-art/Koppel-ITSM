...
$(document).ready(function() {
  ...
  // Type filter (buttons)
    ...
    $('#inventoryTable tbody').on('click', 'tr', function(e) {
        ...
        var ram = $row.data('ram');
        var rom = $row.data('rom');
        ...
        if(cpu || ram || rom){
            ...
            $('#modalKey').text($row.data('key') || '—');
            $('#modalAntivirus').text($row.data('antivirus') || '—');
            $('#modalCompName').text($row.data('compname') || '—');
...
$('#inventoryTable tbody').on('contextmenu', 'tr', function(e) {
        ...
        if (qty > 0) {
            ...
            $('#issueLink').attr('href', '?page=inventory/crud/issue_item&item_id=' + encodeURIComponent(itemId)).show();
        } else {
