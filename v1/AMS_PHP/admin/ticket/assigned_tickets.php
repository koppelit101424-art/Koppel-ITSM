...
<?php include 'ticket_header.php'; ?>
 <?php include 'ticket_stats_card.php'; ?>

<div class="card">
<div class="card-header">My Assigned Tickets</div>
...
if(id) window.location.href=BASE_URL+'/view_ticket.php?ticket_id='+id;
});
...
e.stopPropagation();
let select=$(this);
let id=select.data('ticket-id');
let current=select.data('current');
let val=select.val();
if(current===val)return;

if(!confirm("Change ticket status?")){
select.val(current);
return;
}

$.post(BASE_URL+'update_ticket_status.php',
{ticket_id:id,status:val},
function(){
select.data('current',val);
select.removeClass(function(i,c){
return (c.match(/status-\S+/g)||[]).join(' ');
});
select.addClass('status-'+val.replace(/ /g,'-'));
