...
<option value="<?= $c ?>" <?= $ticket['ticket_category']===$c?'selected':'' ?>>
<?= ucfirst($c) ?>
</option>
<?php endforeach; ?>
</select>
...
<select class="form-select mb-3 live-update" data-field="status" id="statusSelect">
...
<option value="<?= $u ?>" <?= ($ticket['urgency']??'medium')===$u?'selected':'' ?>>
<?= ucfirst($u) ?>
</option>
<?php endforeach; ?>
</select>
...
<option value="<?= $i ?>" <?= ($ticket['impact']??'moderate')===$i?'selected':'' ?>>
<?= ucfirst($i) ?>
</option>
<?php endforeach; ?>
</select>
...
<option value="<?= $r ?>" <?= ($ticket['pending_reason']??'None')===$r?'selected':'' ?>>
<?= $r ?>
</option>
<?php endforeach; ?>
</select>
...
if (assignBtn) {
    assignBtn.addEventListener('click', () => {
        ...
        .then(data => {
            if (data.success) {
                document.getElementById('assigneeContainer').innerHTML = data.assignee_name;
            }
        });
    });
}
