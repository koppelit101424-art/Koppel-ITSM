...
// FETCH DEPARTMENT FROM DB (FIX)
$user_id = $_SESSION['user_id'];
$deptStmt = $conn->prepare("SELECT department FROM user_tb WHERE user_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        ...
        $sql = "INSERT INTO request_tb 
                (lmr_no, requestor, department, created_by, item, description, quantity, UoM, date_needed, remarks, status, date_created, date_updated)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())";
...
<?= $success ?>
<a href="requests.php" class="alert-link">View Requests</a>
</div>
<?php endif; ?>
...
function addItemRow() {
    ...
    <div class="row">
        ...
        <div class="col-md-2">
            ...
            <input type="text" class="form-control" name="date_needed[]" value="ASAP" required>
        </div>
    </div>
    <div class="row mt-2">
        <div class="col-md-6">
